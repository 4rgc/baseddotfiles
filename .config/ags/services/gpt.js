...
const initMessages = [
  ...
  { role: "assistant", content: "- Got it!" },
  {
    role: "user",
    content:
      '"He rushed to where the event was supposed to be hold, he didn\'t know it got canceled"',
  },
  {
    role: "assistant",
    content:
...
class GPTMessage extends Service {
  ...
  set content(content) {
    this._content = content;
    if (
      this._content.length - this._lastContentLength >=
      userOptions.ai.charsEachUpdate
    ) {
      this.notify("content");
      this.emit("changed");
...
class GPTService extends Service {
  ...
  _currentProvider = PROVIDERS[userOptions.ai.defaultGPTProvider]
